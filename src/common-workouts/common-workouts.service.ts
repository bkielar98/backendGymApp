...
export class CommonWorkoutsService {
  ...
  async getIndexForUser(userId: number, commonWorkoutId: number) {
    const commonWorkout = await this.getCommonWorkoutIndexEntityForUser(
      userId,
      commonWorkoutId,
    );
  ...
  async getBlockForUser(
    ...
    const commonWorkout = await this.getCommonWorkoutBlockEntityForUser(
      userId,
      commonWorkoutId,
      blockId,
    );
  ...
  async addBlock(
    ...
    this.ensureCommonWorkoutExerciseLimit(
      existingExerciseCount + participants.length,
    );
    this.ensureCommonWorkoutTotalSetsLimit(
      existingSetsCount + setsCount * participants.length,
    );
    ...
    const blocksToShift = shiftedBlocks.filter(
      (block) => block.order > insertOrder,
    );
    ...
    const previousSetsByUserId =
      typeof exercise?.id === 'number' && setsCount > 0
        ? await this.getPreviousSetsByUserIdForExercise(
            participants.map((participant) => participant.userId),
            exercise.id,
          )
        : new Map<number, Map<number, WorkoutSet>>();
    const commonWorkoutExercises = participants.map((participant) =>
      this.commonWorkoutExerciseRepository.create({
    ...
    const savedExercises = await this.commonWorkoutExerciseRepository.save(
      commonWorkoutExercises,
    );
  ...
  async addSet(userId: number, exerciseOrder: number) {
    ...
    this.ensureCommonWorkoutTotalSetsLimit(
      (await this.countCommonWorkoutSets(
  ...
  private async getWorkoutExerciseResponse(
    ...
    const exercise = await this.getExerciseByIdForUser(
      userId,
      commonWorkoutId,
      exerciseOrder,
    );
  ...
  private countCommonWorkoutSets(commonWorkoutId: number) {
  ...
  private async getCommonWorkoutIndexEntityForUser(
    userId: number,
    commonWorkoutId: number,
  ) {
    const commonWorkout = await this.commonWorkoutRepository.findOne({
  ...
  private async getCommonWorkoutBlockEntityForUser(
    userId: number,
    commonWorkoutId: number,
  ...
  private async attachCommonWorkoutGraph(
    commonWorkout: CommonWorkout,
    includeSets: boolean,
  ) {
