...
export class SchemaFixService implements OnApplicationBootstrap {
  ...
  private async ensureCommonWorkoutSchema() {
    ...
    await this.dataSource.query(
      'ALTER TABLE "common_workout_exercise" ADD COLUMN IF NOT EXISTS "participantId" integer NULL',
    );
    await this.dataSource.query(`
      DO $$
      BEGIN
  ...
  private async migrateCommonWorkoutExercisesToParticipantScopedEntries() {
    ...
    this.logger.log(
      `Common workout exercise migration verified for ${legacyExercises.length} legacy entries`,
    );
  }
